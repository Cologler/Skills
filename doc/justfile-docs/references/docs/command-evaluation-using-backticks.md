...
See the [Strings](strings.html) section for details on unindenting.

Backticks may not start with `#!`. This syntax is reserved for a future
upgrade.

The [`shell(…)` function](built-in-functions.html#external-commands) provides a more general mechanism
